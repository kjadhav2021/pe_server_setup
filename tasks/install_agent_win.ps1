...
$server          = 'puppet-ent-server.platform9.puppet.net'
$port            = '8140'
$pe_version      = '2019.8.4'
$enable_bulk_pluginsync = [System.Convert]::ToBoolean('true')
...
$bulk_pluginsync_source = "https://${server}:$port/packages/bulk_pluginsync.zip"
$bulk_pluginsync_dest   = Join-Path ([System.IO.Path]::GetTempPath()) "bulk_pluginsync.zip"
...
function DownloadFromPuppetServer ($source, $dest, $error_message) {
...
function DownloadPuppet {
  ...
  DownloadFromPuppetServer $msi_source $msi_dest "Failed to download the Puppet Agent installer: $msi_source. Does the Puppet Master have the $pe_repo_class class applied to it?"
}

# https://stackoverflow.com/questions/37814037/how-to-unzip-a-zip-file-with-powershell-version-2-0/37814462#37814462
function Expand-BulkPluginsync-ZipFile($zipfile, $cache_destination) {
  if ($PSVersionTable.PSVersion.Major -ge 4) {
    Write-Verbose "Detected Powershell version 4+... Using system.io.compression.filesystem"
    if (Test-Path $cache_destination -PathType Container) {
      Write-Verbose "Detected existing pluginsync cache directory.  Removing before zipfile extraction."
      Get-ChildItem $cache_destination -Recurse | Remove-Item -Recurse
    }
    Add-Type -Assembly "system.io.compression.filesystem"
    [io.compression.zipfile]::ExtractToDirectory($zipfile, $cache_destination)
  } else {
    Write-Verbose "Detected Powershell less than version 4"
    ...
    foreach($item in $zip.items())
    {
      ...
      #20 is the combo of the above
      $shell.Namespace($cache_destination).copyhere($item,20)
    }
  }
}

function BulkPluginsync {
  Write-Output "Downloading bulk_pluginsync zipfile on $env:COMPUTERNAME..."
  DownloadFromPuppetServer $bulk_pluginsync_source $bulk_pluginsync_dest "Failed to download the bulk_pluginsync zip: $bulk_pluginsync_source."

  Write-Output "Extracting bulk_pluginsync zipfile on $env:COMPUTERNAME..."
  $puppet_dir = "C:\ProgramData\PuppetLabs\puppet"
  $pluginsync_cache_dir = "$puppet_dir\cache"
  if (!(Test-Path $pluginsync_cache_dir -PathType Container)){
    New-Item -Path $puppet_dir -Name "cache" -ItemType "directory"
  }

  Expand-BulkPluginsync-ZipFile $bulk_pluginsync_dest $pluginsync_cache_dir
}
...
if ($enable_bulk_pluginsync){
  BulkPluginsync -ErrorAction SilentlyContinue
}
InstallPuppet
